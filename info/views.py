...
def marks_confirm(request, marks_c_id):
    ...
    for s in cl.student_set.all():
        ...
        m, created = sc.marks_set.get_or_create(name=mc.name, defaults={'marks1': mark})
        
        if not created:
            # If the instance already exists, update the mark
            m.marks1 = mark
            m.save()
    mc.status = True
    mc.save()
...
def edit_marks(request, marks_c_id):
    ...
    for stud in stud_list:
        ...
        m, created = sc.marks_set.get_or_create(name=mc.name, defaults={'marks1': 0})
